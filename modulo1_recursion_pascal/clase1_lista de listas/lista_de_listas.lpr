...
end;    {  IDEA DE CARGAR LISTA RECURSIVO
 procedure  cargar_datos(ls,lc);
 begin
if(ls<>nil)and(marcaact=ls^.dato.marca)do begin
  agregarAdelante(lc^.datos.puntero, ls^.dato);
  ls:=ls^.sig;
  cargar_datos(ls,lc);
end;
...
procedure cargarlistacompleja_recursiva(ls:listasimple; var lc:listacompleja);
var
  marcaact:string;
begin
  if(ls<>nil)do begin
     ...
     cargarlistacompleja_recursiva(ls,lc);
  end;
end;          }
