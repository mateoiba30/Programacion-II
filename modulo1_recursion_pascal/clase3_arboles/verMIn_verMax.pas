...
begin
  if(a=nil)then //si a=nil no hay ningun dato guardado, si a^.HI=nil no apunta a nada. Si apunta a nil no va a querer avanzar, solo a=nil si al principio esta vacio
    VerMax:=-1
  else begin
    if(a^.HD<>nil)then
      VerMax:=VerMax(a^.HD)
...
begin
 ...
 while(e<>0)do begin
  ...
  writeln('El valor minimo es: ',VerMin(a));
  readkey();
  writeln('El valor maximo es: ',VerMax(a));
  readkey();
  readln;
end.
