...
begin
     ...
     readln;      //si fuera una lista de listas:
     {
     al cargar el arbol deberíamos recorrer un nodo de la lista, y dentro de él recorrer su lista de mensajes para ese usuario. una vez hecho eso se le asigna la cantidad de mensajes al nodo del arbol
     cuando recorremos la lista para cargar los datos sabemos que el usuario no se va a repetir
     cuando buscamos el nombre nos movemos en la lista principal. o sea no donde están los mensajes, sinó que vamos solo por donde se guarda el nombre y codigo
