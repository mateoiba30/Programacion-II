...
function buscar_cercano(a:arbol):longint;  //ME DEBERÍA DEVOLVER EL PUNTERO AL NODO, NO SOLO EL DNI
begin
  if(a=nil)then
    buscar_cercano:=-1
  else begin
          ...
          if(a^.hi=nil)then begin//si tiene hijo derecho nomás  (o no tiene ningun hijo ) le copio todos los datos del hijo al padre
            ...
            if(a^.hd=nil)then begin//lo mismo pero con el hijo izquierdo
              ...
              dispose(aux);
                  ...
                  a^.dato.dni:=buscar_cercano(a^.hd); //debería haber copiado no solo el dni
